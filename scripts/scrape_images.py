...
for card in goat_cards:
    ...
    url = f"https://db.ygoprodeck.com/api/v7/cardinfo.php?name={card_name.replace('#', '').replace('&', '')}"
    try:
    ...
    for i, img in enumerate(images):
        path = os.path.join(card_dir, f"{img['id']}_cropped.jpg")
        if os.path.exists(path):
            continue
        try:
            r = requests.get(img['image_url_cropped'])
            if r.status_code == 200:
                with open(path, "wb") as f:
                    f.write(r.content)
        except Exception as e:
            print(f"Failed to download {card_name} image {img['id']}: {e}")